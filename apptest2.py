...
# ✅ 2단계: 여기에 merge_stocks 함수 삽입 ⬇️⬇️⬇️
def merge_stocks(base_stocks, custom_input):
    """기본 종목 + 사용자 입력 종목 병합"""
...
class MAG7BTCVWAPAnalyzer:
    def __init__(self, start_date='2020-01-01', end_date=None, 
                 burn_in_calendar_days=14, custom_stocks=None):
        ...
        self.errors = {}
        # ✅ 커스텀 종목 지원
        self.stocks = custom_stocks if custom_stocks else DEFAULT_STOCKS.copy()
...
def main():
    ...
    with st.sidebar:
        ...
        custom_tickers = st.text_input(
            "🎯 추가 분석 종목 (쉼표로 구분)",
            placeholder="예: PLTR, SOFI, ETH-USD",
            help="야후 파이낸스 티커 형식으로 입력하세요"
        )

        # ✅ 최종 분석 종목 표시
        final_stocks = merge_stocks(DEFAULT_STOCKS, custom_tickers)
        with st.expander(f"📋 분석 대상 종목 ({len(final_stocks)}개)"):
            base_tickers = list(DEFAULT_STOCKS.keys())
            for ticker, name in final_stocks.items():
                if ticker in base_tickers:
                    st.text(f"• {name} ({ticker})")
                else:
                    st.text(f"🆕 {name} ({ticker})")  # 신규 종목 강조
        ...
        if st.button("🚀 데이터 분석 실행", type="primary", use_container_width=True):
            ...
            analyzer = MAG7BTCVWAPAnalyzer(
                start_date=start_date, 
                burn_in_calendar_days=burn_in,
                custom_stocks=final_stocks  # ✅ 추가
            )
